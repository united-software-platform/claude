...
```bash
docker pull ghcr.io/alexgaib/usp-claude-claude:latest
```
...
для `linux/amd64` и `linux/arm64` при изменениях в `docker/claude/`. Имя пакета —
`ghcr.io/<владелец>/<репозиторий>-claude`.
